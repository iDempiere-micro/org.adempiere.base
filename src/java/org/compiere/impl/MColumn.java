...
public class MColumn extends X_AD_Column
{
	...
	public boolean isUUIDColumn() {
		return getColumnName().equals(getUUIDColumnName(getAD_Table().getTableName()));
	}
	...
	*/
	...
	public String getConstraint(String tableName)
	{
		...
		**/
		// IDEMPIERE-965
		if (getColumnName().equals(getUUIDColumnName(tableName))) {
	...
	*/
	...
	public void setSmartDefaults() { // IDEMPIERE-1649 - dup code on Callout_AD_Column.columnName
		...
		// IDEMPIERE-1011
		if (getUUIDColumnName(getAD_Table().getTableName()).equals(getColumnName())) {
			// UUID column
	...
	public static String getForeignKeyConstraintSql(DatabaseMetaData md, String catalog, String schema, String tableName, MTable table, MColumn column, boolean isNoTable) throws Exception
	{
		...
		if (!column.isKey() && !column.getColumnName().equals(getUUIDColumnName(table.getTableName())) && !column.isVirtualColumn())
		{
						...
						if (dbForeignKey.getKeyColumns()[0].equalsIgnoreCase(column.getColumnName()))
						{
							...
							if (primaryKey != null)
							{
								...
								if (dbForeignKey.getDeleteRule() == DatabaseMetaData.importedKeyCascade)
									dbDeleteRule = FKCONSTRAINTTYPE_Cascade;
								else if (dbForeignKey.getDeleteRule() == DatabaseMetaData.importedKeySetNull)
									dbDeleteRule = FKCONSTRAINTTYPE_SetNull;
									...
									dbDeleteRule = FKCONSTRAINTTYPE_NoAction;
								...
								if (fkConstraintType == null) {
									...
									if (fkConstraintType == null) {
										if (   "AD_Client_ID".equals(column.getColumnName())
											...
											fkConstraintType = FKCONSTRAINTTYPE_DoNotCreate;
										else
											fkConstraintType = FKCONSTRAINTTYPE_NoAction;
									}
								}
								if (!fkConstraintType.equals(FKCONSTRAINTTYPE_DoNotCreate))
								{
									...
									if (fkConstraintType.equals(FKCONSTRAINTTYPE_NoAction))
										;
									else if (fkConstraintType.equals(FKCONSTRAINTTYPE_Cascade))
										fkConstraint.append(" ON DELETE CASCADE");
									else if (fkConstraintType.equals(FKCONSTRAINTTYPE_SetNull))
									...
									if (   dbForeignKey.getKeyName().equalsIgnoreCase(column.getFKConstraintName())
										&& (   (dbForeignKey.getDeleteRule() == DatabaseMetaData.importedKeyCascade && FKCONSTRAINTTYPE_Cascade.equals(column.getFKConstraintType()))
										    || (dbForeignKey.getDeleteRule() == DatabaseMetaData.importedKeySetNull && FKCONSTRAINTTYPE_SetNull.equals(column.getFKConstraintType()))
										    || (dbForeignKey.getDeleteRule() == DatabaseMetaData.importedKeyNoAction && FKCONSTRAINTTYPE_NoAction.equals(column.getFKConstraintType()))
										    || (dbForeignKey.getDeleteRule() == DatabaseMetaData.importedKeyRestrict && FKCONSTRAINTTYPE_NoAction.equals(column.getFKConstraintType()))
										   )
									   ) {
										// nothing changed
										return "";
	...
	{		
		if (!column.isKey() && !column.getColumnName().equals(getUUIDColumnName(table.getTableName())))
		{
			String fkConstraintType = column.getFKConstraintType();
			if (fkConstraintType == null)
				fkConstraintType = FKCONSTRAINTTYPE_NoAction;
