...
public class MOrder extends X_C_Order implements DocAction
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (getM_Warehouse_ID() == 0)
		{
			...
			{
				throw new FillMandatoryException(I_C_Order.COLUMNNAME_M_Warehouse_ID);
			}
		}
		...
		BigDecimal previousProcessedOn = (BigDecimal) get_ValueOld(I_C_Order.COLUMNNAME_ProcessedOn);
		if (! newRecord && previousProcessedOn != null && previousProcessedOn.signum() > 0) {
			int previousDocTypeID = (Integer) get_ValueOld(I_C_Order.COLUMNNAME_C_DocTypeTarget_ID);
			MDocType previousdt = MDocType.get(getCtx(), previousDocTypeID);
			if (is_ValueChanged(I_C_Order.COLUMNNAME_C_DocType_ID) || is_ValueChanged(I_C_Order.COLUMNNAME_C_DocTypeTarget_ID)) {
				if (previousdt.isOverwriteSeqOnComplete()) {
					log.saveError("Error", Msg.getMsg(getCtx(), "CannotChangeProcessedDocType"));
					return false; 
				}
			}
			if (is_ValueChanged(I_C_Order.COLUMNNAME_DateOrdered)) {
		...
		if (!newRecord && (is_ValueChanged(I_C_Order.COLUMNNAME_M_PriceList_ID) || is_ValueChanged(I_C_Order.COLUMNNAME_DateOrdered))) {
			int cnt = DB.getSQLValueEx(get_TrxName(), "SELECT COUNT(*) FROM C_OrderLine WHERE C_Order_ID=? AND M_Product_ID>0", getC_Order_ID());
			if (cnt > 0) {
				if (is_ValueChanged(I_C_Order.COLUMNNAME_M_PriceList_ID)) {
					log.saveError("Error", Msg.getMsg(getCtx(), "CannotChangePl"));
					return false;
				}
				if (is_ValueChanged(I_C_Order.COLUMNNAME_DateOrdered)) {
					MPriceList pList =  MPriceList.get(getCtx(), getM_PriceList_ID(), null);
					MPriceListVersion plOld = pList.getPriceListVersion((Timestamp)get_ValueOld(I_C_Order.COLUMNNAME_DateOrdered));
					MPriceListVersion plNew = pList.getPriceListVersion((Timestamp)get_Value(I_C_Order.COLUMNNAME_DateOrdered));
		...
		if (! recursiveCall && (!newRecord && is_ValueChanged(I_C_Order.COLUMNNAME_C_PaymentTerm_ID))) {
			recursiveCall = true;
			try {
	...
	public String prepareIt()
	{
		...
		if (isSOTrx() && getDeliveryViaRule().equals(DELIVERYVIARULE_Shipper))
		{
			...
			{
				m_processMsg = "@FillMandatory@" + Msg.getElement(getCtx(), I_C_Order.COLUMNNAME_M_Shipper_ID);
				return DocAction.STATUS_Invalid;
			}
