...
public class MInOut extends X_M_InOut implements DocAction
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (!getMovementType().contentEquals(MOVEMENTTYPE_CustomerReturns) && isSOTrx() && getC_Order_ID() == 0 && getM_RMA_ID() == 0)
		{
	...
	protected void checkMaterialPolicy(MInOutLine line,BigDecimal qty)
	{
		...
		if (product != null && line.getM_AttributeSetInstance_ID() == 0)
		{
			...
			if (getMovementType().compareTo(MOVEMENTTYPE_VendorReceipts) == 0 )
			{
			...
			} else if (getMovementType().compareTo(MOVEMENTTYPE_CustomerReturns) == 0){
			...
			else if(getMovementType().compareTo(MOVEMENTTYPE_VendorReturns) == 0 || getMovementType().compareTo(MOVEMENTTYPE_CustomerShipment) == 0)
			{
				String MMPolicy = product.getMMPolicy();
